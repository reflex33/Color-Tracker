...
namespace tracking
{
    public class color_finder
    {
        ...
        public void add_color(Color name, float min_hue, float max_hue, float min_saturation, float max_saturation, float min_brightness, float max_brightness)
        {
            hsb_filter filter = new hsb_filter();
            filter.min_hue = min_hue;
            filter.max_hue = max_hue;
            filter.min_saturation = min_saturation;
            filter.max_saturation = max_saturation;
            filter.min_brightness = min_brightness;
            filter.max_brightness = max_brightness;
            add_color(name, filter);
        }
        public void add_color(Color name, hsb_filter filter)
        {
        ...
        public List<found_color_rectangle> find_colors(out List<found_color_center> output_centers, Bitmap new_image)
        {
            color_locations.Clear();  // Clear old results
            hsb_image converted_image = new hsb_image(new_image);
            ...
            {
                copies_of_image[i] = AForge.Imaging.Image.Clone(new_image);
                threads[i] = new BackgroundWorker();
                threads[i].DoWork += new DoWorkEventHandler(thread_find_colors);
                thread_done[i] = new AutoResetEvent(false);
                Tuple<Bitmap, hsb_image, color_to_find, AutoResetEvent> work_data = new Tuple<Bitmap, hsb_image, color_to_find, AutoResetEvent>(copies_of_image[i], converted_image, colors[i], thread_done[i]);
                threads[i].RunWorkerAsync(work_data);
            }
        ...
        {
            Tuple<Bitmap, hsb_image, color_to_find, AutoResetEvent> work_data = (Tuple<Bitmap, hsb_image, color_to_find, AutoResetEvent>)e.Argument;
